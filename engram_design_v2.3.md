...
- 앱이 다시 시작되면 raw에만 있고 아직 successful extraction run이 없는 turn은 startup catch-up으로 다시 큐에 올라간다.
...
현재 구현된 최소형:
- startup catch-up은 `successful extraction run` 존재 여부를 gap 판단 기준으로 사용한다.
- raw에만 있고 아직 successful extraction run이 없는 turn은 startup 시 `QueueItem`으로 다시 enqueue한다.
