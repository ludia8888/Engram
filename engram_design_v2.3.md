...
- 앱이 다시 시작되면 raw에만 있고 `events.source_turn_id`로 아직 연결되지 않은 turn은 startup catch-up으로 다시 큐에 올라간다.
...
1. raw manifest 확인
2. 마지막 raw turn과 마지막 successful extraction run 비교
3. gap이 있으면 queue에 다시 넣음
4. pending dirty range가 있으면 projector 재개

현재 구현된 최소형:
- background extractor가 아직 없으므로 "successful extraction run" 대신 `events.source_turn_id` 존재 여부를 gap 판단 기준으로 사용한다.
- raw에만 있고 아직 어떤 canonical event에도 연결되지 않은 turn은 startup 시 `QueueItem`으로 다시 enqueue한다.
